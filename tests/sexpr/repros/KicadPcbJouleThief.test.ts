...
import { EmbeddedFonts, KicadPcb, SxClass } from "lib/sexpr"
...
test("kicad_pcb: Joule Thief parses gr_rect and embedded_fonts", async () => {
  const original = await Bun.file("tests/assets/joule-thief.kicad_pcb").text()
  ...
  expect(pcb.graphicRects.length).toBeGreaterThan(0)
  expect(pcb.embeddedFonts).toBeInstanceOf(EmbeddedFonts)
  expect(pcb.embeddedFonts?.enabled).toBe(false)
  ...
  expectEqualPrimitiveSExpr(roundTripPrimitive, originalPrimitive, {
    path: "tests/assets/joule-thief.kicad_pcb",
  })
})
